...
last = 0
true_successor= []
testfile_name = "C:\\workspace\\mycorpus\\test-files\\test-1.txt"
csvfilename = "ADI_trigram_random_result_graph_test-sample.csv"
probfilename = "ADI_trigram_random_probability-matrix-sample.csv"
#Extracting Heads and Tails of all clusters and converting to words

print("File name: test1.txt")
...
block_sequence_order = [11,14,4,15,13,6,5,3,10,12,8,9,0,2,1,7]
...
outfile = open("Head-Tail-of-all-clusters.txt",'w')   
#Extracting Heads and Tails of all clusters and converting to words
for cno in range(0,no_of_clusters):
    ...
    print("Cluster",cno,", Block",block_sequence_order[cno],":","\nHEAD:",head_words[cno],"\nTAIL:",tail_words[cno],end = '\n')
    outfile.write("Cluster"+str(cno)+", Block"+str(block_sequence_order[cno])+":\nHEAD:"+str(head_words[cno])+"\nTAIL:"+str(tail_words[cno])+'\n')
    #print("Block",block_sequence_order[cno],":","\nHEAD:",head_words[cno],"\nTAIL:",tail_words[cno],end = '\n')
    head = tail = ""
infile.close()
outfile.close()
...
outfile = open("TailTokens-join-HeadTokens.txt",'w')
#validating and Concatenating tail end word with head start word 
for i in range(0,no_of_clusters):
    #print("\ncluster:",i,"\n")
    #print("\nThe number of words in all tails(i.e length of each tail) is :", length, end = '\n')
    for j in range(0, no_of_clusters):       
        ...
        outfile.write("Cluster"+str(i)+"_Tail join Cluster"+str(j)+"_Head:\n"+str(all_words)+"\n")
        word_sequences.append(all_words)
    outfile.write("\n")
outfile.close()    


#Extracting a phrase across the blocks of all combinations ( mid , mid-1, mid+1)
outfile = open("PhraseTokens.txt",'w')
j=0
#outfile.write("Cluster "+str(j)+",Block "+str(block_sequence_order[j])+":\n")
for i in range(0,len(word_sequences)):
    ...
    outfile.write("Phrase"+str(j)+"-"+str(i%no_of_clusters)+":\t"+str(word_sequences[i])+"\n")
...
test_sentence_trigrams = [ list( ngrams(pad_both_ends(word_sequences[i], n=2),3) ) for i in range(len(word_sequences)) ] 
model_filepath = 'ADI_trigram_model.pkl'
...
for i in range(len(test_sentence_trigrams)):
    product = 1
    for t in test_sentence_trigrams[i]:
        product = product*lm_model.score(t[2],[t[0],t[1]])
    row.append(product)
